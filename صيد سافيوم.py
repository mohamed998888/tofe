import os
os.system('pip install wbsocket')
os.system('pip install gzip')
os.system('pip install names')
os.system('clear')
my_secret = input(' Enter Token | توكن بوتك : ')
...
def Start(message):
    ...
    control = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton(' اضغط هنا لبدأ الصيد  ✅',callback_data='StartSafum'))
    control.add(types.InlineKeyboardButton(' المطور 👨🏻‍💻',url='Lx0b2.t.me'),types.InlineKeyboardButton('قناة المطور',url='Pythonln.t.me'))
...
def CallingButtons(call):
    ...
    if call.data=='StartSafum':
        with open('StartSafum.txt','a') as a:
            pass
        d = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton(' جاري التحميل .⏳',callback_data='h'))
        d.add(types.InlineKeyboardButton(' توقف 🛑',callback_data='Stop'))
        bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text='↯︙انتظر ...\n↯︙جاري انشاء الحساب  ...',reply_markup=d);
